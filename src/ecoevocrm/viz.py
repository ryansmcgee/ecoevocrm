...
def stacked_abundance_plot(system, ax=None, palette='hls', root_color='#AAAAAA', 
                           apply_palette_depth=0, shuffle_palette=True, color_step_start=0.1, color_step_slope=0.02, color_step_min=0.01,
    ...
    num_palette_types = 0
    for lineage_id in system.strain_pool.lineage_ids:
    ...
    type_colors = [root_color for i in range(system.strain_pool.num_types)]
    ...
    def color_subtree(d, parent_color, depth, next_palette_color_idx):
        ...
        for lineage_id, descendants in d.items():
            type_idx       = np.where(system.strain_pool.lineage_ids == lineage_id)[0][0]
            if(depth == apply_palette_depth):
                ...
                color_step_scale = color_step_start - min(color_step_slope*(depth-1), color_step_min)
    ...
    color_subtree(system.strain_pool.phylogeny, parent_color=root_color, depth=0, next_palette_color_idx=0)

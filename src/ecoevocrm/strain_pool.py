...
class StrainPool():
    ...
    def __init__(self, num_types     = None,
        ...
        if(isinstance(sigma, (list, np.ndarray))):
            ...
            utils.error("Error in StrainPool __init__(): Number of types and traits must be specified by providing a) a sigma matrix, or b) both num_types and num_traits values.")
    ...
    def generate_mutant_pool(self, sigma=None, biochemistry=None, has_phylogeny=False):
        sigma = self.sigma if sigma is None else sigma
        #----------------------------------
        ...
        #----------------------------------
        mutant_pool = StrainPool(sigma=sigma_mut, b=b_mut, k=k_mut, eta=eta_mut, l=l_mut, g=g_mut, c=c_mut, chi=chi_mut, mu=mu_mut, has_phylogeny=has_phylogeny)
        mutant_pool.update_phenotypic_costs(biochemistry=biochemistry)
        #----------------------------------
        return mutant_pool


    def add_type(self, strain_pool=None, sigma=None, b=None, k=None, eta=None, l=None, g=None, c=None, chi=None, mu=None, index=None, parent_index=None, parent_id=None):
        ...
        if(strain_pool is not None):
            if(isinstance(strain_pool, StrainPool)):
                new_strain_pool = strain_pool
            else:
                utils.error(f"Error in StrainPool add_type(): strain_pool argument expects object of StrainPool type.")
        else:
            new_strain_pool = StrainPool(sigma=sigma if sigma is not None else self.sigma[ref_type_idx], 
        ...
        if(self.num_traits != new_strain_pool.num_traits): 
            utils.error(f"Error in StrainPool add_type(): The number of traits for added types ({new_strain_pool.num_traits}) does not match the number of strain pool traits ({self.num_traits}).")
        #----------------------------------
        self.sigma = np.insert(self.sigma, new_type_idx, new_strain_pool.sigma, axis=0)
        self.b     = np.insert(self.b,     new_type_idx, new_strain_pool.b,     axis=0)
        self.k     = np.insert(self.k,     new_type_idx, new_strain_pool.k,     axis=0)
        self.eta   = np.insert(self.eta,   new_type_idx, new_strain_pool.eta,   axis=0)
        self.l     = np.insert(self.l,     new_type_idx, new_strain_pool.l,     axis=0)
        self.g     = np.insert(self.g,     new_type_idx, new_strain_pool.g,     axis=0)
        self.c     = np.insert(self.c,     new_type_idx, new_strain_pool.c,     axis=0)
        self.mu    = np.insert(self.mu,    new_type_idx, new_strain_pool.mu,    axis=0)
        self.chi   = np.insert(self.chi,   new_type_idx, new_strain_pool.chi,   axis=0)
        #----------------------------------
        ...
        self.type_ids = np.array([self.get_type_id(i) for i in range(self.num_types)])
        #----------------------------------
        for i in range(new_type_idx, new_type_idx+new_strain_pool.num_types):
    ...
    def get_type(self, index=None, type_id=None):
        ...
        if(index is None and type_id is None):
            utils.error(f"Error in StrainPool get_type(): A type index or type id must be given.")
        type_idx = np.where(self.type_ids==type_id)[0] if type_id is not None else index
        return StrainPool(sigma=self.sigma[type_idx], b=self.b[type_idx], k=self.k[type_idx], 
